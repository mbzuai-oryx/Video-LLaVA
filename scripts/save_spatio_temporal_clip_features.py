...
def load_video(vis_path, num_frm=100, shape=(224,224)):
...
def parse_args():
    ...
    parser.add_argument("--llava", required=True, choices=['1.1', '1.5'], help="LLaVA version")
...
def main():
    ...
    if args.llava=="1.1":
        ...
        image_processor = CLIPImageProcessor.from_pretrained("openai/clip-vit-large-patch14-336", torch_dtype=torch.float16)
        vision_tower = CLIPVisionModel.from_pretrained("openai/clip-vit-large-patch14-336", torch_dtype=torch.float16,
                                                    low_cpu_mem_usage=True).cuda()
        vision_tower.eval()
        frame_size = (336,336)
    ...
    for video_name in tqdm(all_videos):
        ...
        if os.path.exists(f"{clip_feat_path}/{video_id}.pkl"):  # Check if the file is already processed
            continue
        try:
            video = load_video(video_path, shape=frame_size)
            video_tensor = image_processor.preprocess(video, return_tensors='pt')['pixel_values']
            video_tensor = video_tensor.half()

            n_chunk = len(video_tensor)
            video_features = torch.FloatTensor(n_chunk, int(frame_size[0]//14)**2, 1024).fill_(0) # (256,1024) or (576,1024)
